...
ROBOT_DEFAULT_SEND_SPEED = 0.25
ROBOT_SAFE_JOYSTICK_MAX = 45
ROBOT_MIN_JOYSTICK_GAP_SEC = 0.12
LIVE_SAFE_BUTTONS = {"Start", "Stop", "A", "X"}

_ROBOT_EXECUTOR = ThreadPoolExecutor(max_workers=1)


def _error_response(
    message: str, status: int, *, code: str | None = None, details: str | None = None
):
...
def request_entity_too_large(_err):
    return _error_response(
        "Uploaded file is too large. Max size is 50 MB.", 413, code="upload_too_large"
    )
...
def build_events(audio_path: str, seed: int = 7) -> tuple[list[dict], float, float]:
    ...
    tempo = (
        float(np.atleast_1d(tempo_raw)[0]) if np.size(np.atleast_1d(tempo_raw)) else 0.0
    )
    beat_times = librosa.frames_to_time(beat_frames, sr=sr)

    if len(beat_times) < 4:
        app.logger.info(
            "Sparse beat detection (%s beats); using fallback beat grid for %s",
            len(beat_times),
            audio_path,
        )
    ...
    for t in beat_times:
        beat_strengths.append(
            (
                _sample_feature(rms_t, rms_z, float(t)),
                _sample_feature(onset_t, onset_z, float(t)),
            )
        )
    ...
    for i, t in enumerate(beat_times[:-1]):
        ...
        if (
            onset_i >= strong_threshold
            and long_move_allowed
            and gap > 0.42
            and bar_pos in (2, 6)
        ):
        ...
        if section == "high":
            ...
            events.append(
                {
                    "t": round(t + gap * 0.55, 3),
                    "kind": "joystick",
                    "payload": [int(-x // 2), 0],
                }
            )
            events.append(
                {"t": round(t + gap * 0.90, 3), "kind": "joystick", "payload": [0, 0]}
            )
        elif section == "mid":
            ...
            events.append({"t": round(t, 3), "kind": "joystick", "payload": [x, yv]})
            events.append(
                {"t": round(t + gap * 0.65, 3), "kind": "joystick", "payload": [0, 0]}
            )
        else:
            x = 20 if (i % 2 == 0) else -20
            events.append({"t": round(t, 3), "kind": "joystick", "payload": [x, 8]})
            events.append(
                {"t": round(t + gap * 0.70, 3), "kind": "joystick", "payload": [0, 0]}
            )
    ...
    events.append(
        {"t": round(duration + 0.10, 3), "kind": "joystick", "payload": [0, 0]}
    )
...
def _clamp_axis(v: int) -> int:
    return max(-ROBOT_SAFE_JOYSTICK_MAX, min(ROBOT_SAFE_JOYSTICK_MAX, int(v)))


def _with_safe_bookends(events: list[dict]) -> list[dict]:
    out = list(events)

    if not out or out[0]["kind"] != "button" or out[0]["payload"] != "Start":
        out.insert(0, {"t": 0.0, "kind": "button", "payload": "Start"})
        out.insert(1, {"t": 0.15, "kind": "joystick", "payload": [0, 0]})

    last_t = float(out[-1]["t"])
    if out[-1]["kind"] != "button" or out[-1]["payload"] != "Stop":
        out.append(
            {"t": round(last_t + 0.10, 3), "kind": "joystick", "payload": [0, 0]}
        )
        out.append({"t": round(last_t + 0.20, 3), "kind": "button", "payload": "Stop"})
...
def _validate_send_speed(raw_value: object) -> float:
    if raw_value is None:
        return ROBOT_DEFAULT_SEND_SPEED
    ...
    if speed < ROBOT_MIN_SEND_SPEED or speed > ROBOT_MAX_SEND_SPEED:
        raise ValueError(
            f"send_speed must be between {ROBOT_MIN_SEND_SPEED:.2f} and {ROBOT_MAX_SEND_SPEED:.2f}."
        )
    return speed


def _validate_robot_events(
    events: object, *, live_mode: bool = False, allow_aggressive: bool = False
) -> list[dict]:
    ...
    for raw in events:
        ...
        if kind == "joystick":
            ...
            if live_mode:
                ...
                y = _clamp_axis(y)
                gap = t - last_joy_t
                if gap < ROBOT_MIN_JOYSTICK_GAP_SEC:
                    raise ValueError(
                        f"joystick changes too fast ({gap:.3f}s gap, min {ROBOT_MIN_JOYSTICK_GAP_SEC}s). "
                        "Add more time between joystick events or use a slower send_speed."
                    )
            last_joy_t = t
        ...
        if kind == "button":
            ...
            if live_mode and not allow_aggressive and label not in LIVE_SAFE_BUTTONS:
                raise ValueError(
                    f"button '{label}' not allowed in live mode. "
                    f"Allowed: {', '.join(sorted(LIVE_SAFE_BUTTONS))}. "
                    "Set allow_aggressive=true to enable all buttons."
                )
    ...
    if normalized[-1]["t"] > ROBOT_MAX_SCRIPT_SECONDS:
        raise ValueError(
            f"last event exceeds max timeline ({ROBOT_MAX_SCRIPT_SECONDS}s)."
        )
    return normalized


def _send_event_timeline_to_robot(
    events: list[dict], base_url: str, send_speed: float = 1.0
) -> dict:
    ...
    for event in events:
        ...
        if event["kind"] == "joystick":
            ...
            dispatches.append(
                {"t": event["t"], "kind": "button", "payload": label, "status": status}
            )
...
def analyze_audio():
    if "audio" not in request.files:
        return _error_response(
            'No audio file uploaded. Use form field name "audio".',
            400,
            code="missing_audio",
        )
    ...
    if suffix.lower() not in ALLOWED_AUDIO_SUFFIXES:
        return _error_response(
            "Unsupported audio format. Allowed: mp3, wav, ogg, m4a, flac.",
            400,
    ...
    try:
        ...
        return _error_response(
            "Audio analysis failed. Verify the file is a valid, non-corrupt audio clip.",
            400,
...
def send_to_robot():
    ...
    allow_aggressive = bool(payload.get("allow_aggressive", False))
    try:
        raw_events = payload.get("events")
        if live_mode:
            raw_events = _with_safe_bookends(raw_events)
        events = _validate_robot_events(
            raw_events, live_mode=live_mode, allow_aggressive=allow_aggressive
        )
        send_speed = _validate_send_speed(payload.get("send_speed"))
    except (TypeError, ValueError) as err:
        return _error_response(
            f"Invalid robot send request: {err}", 400, code="invalid_robot_send_request"
        )
        ...
        return jsonify(
            {
                ...
                "timeline_seconds": round(
                    float(events[-1]["t"]) - float(events[0]["t"]), 3
                ),
                "send_speed": send_speed,
            }
        )

    future = _ROBOT_EXECUTOR.submit(
        _send_event_timeline_to_robot, events, base_url, send_speed
    )
    try:
        result = future.result(timeout=ROBOT_MAX_SCRIPT_SECONDS + 15.0)
    except FutureTimeoutError:
        app.logger.exception(
            "Robot dispatch timed out for %s events to %s", len(events), base_url
        )
        return _error_response(
            "Robot dispatch timed out.",
            504,
            code="robot_dispatch_timeout",
            details=f"Timeline execution exceeded {ROBOT_MAX_SCRIPT_SECONDS + 15.0:.0f}s timeout.",
        )
        ...
        return _error_response(
            ...
            details=str(getattr(err, "reason", err)),
        )
    except Exception as err:
        app.logger.exception(
            "Robot dispatch failed for %s events to %s", len(events), base_url
        )
        return _error_response(
            "Robot dispatch failed unexpectedly.",
            500,
            code="robot_dispatch_failed",
...
def emergency_stop():
    ...
    base_url = _normalize_robot_base_url(payload.get("base_url"))
    try:
        _http_robot_get(base_url, "/joystick", {"x": 0, "y": 0})
        _http_robot_get(base_url, "/button", {"label": "Stop"})
        return jsonify({"ok": True})
    except Exception:
        app.logger.exception("Emergency stop failed for %s", base_url)
        return _error_response(
            "Emergency stop failed.", 500, code="emergency_stop_failed"
        )
